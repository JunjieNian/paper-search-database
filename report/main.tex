...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small 数据库引论实验课\,——\,课程PJ报告}
...
\vspace{2cm}
{\Large 数据库引论实验课\,——\,课程PJ报告}

\vspace{3cm}
\begin{tabular}{rl}
\textbf{课程名称：} & 数据库引论实验课 \\[0.5em]
\textbf{选题方向：} & 论文搜索与推荐 \\[0.5em]
\textbf{姓\qquad 名：} & \underline{\hspace{6em}} \\[0.5em]
\textbf{学\qquad 号：} & \underline{\hspace{6em}} \\[0.5em]
\textbf{提交日期：} & 2026 年 \quad 月 \quad 日 \\
\end{tabular}
...
随着学术论文数量的爆炸式增长，研究者在海量文献中高效检索并发现有价值的论文成为一项重要需求。传统的关键词匹配搜索仅能捕获字面重合，无法理解查询的语义意图。本项目面向数据库与数据管理领域，构建了一套融合\textbf{向量语义检索}、\textbf{基于Embedding质心的个性化推荐}、\textbf{可选精排}以及\textbf{大语言模型学术问答}的全栈论文搜索与推荐系统。
...
\item \textbf{对话流程}：前端发送多轮对话 $\to$ 业务层注入用户最近5篇已读论文作为系统提示词 $\to$ 转发至算法层 $\to$ 调用DashScope Qwen兼容接口 $\to$ SSE流式返回 $\to$ 前端实时渲染Markdown。
\end{enumerate}
...
\centering
\begin{tikzpicture}[
    entity/.style={draw, rectangle, rounded corners=4pt, minimum width=4cm, minimum height=0.8cm, fill=blue!10, font=\small\bfseries},
...
\node[entity] (papers) at (8, 0) {papers};
\node[entity] (clicks) at (4, -3) {user\_clicks};
\node[entity] (history) at (0, -3) {search\_history};

% Attributes - users
\node[attr, anchor=east] at (-0.3, 1.2) {
  \begin{tabular}{l}
  \underline{id}: INT (PK) \\
...
% Attributes - papers
\node[attr, anchor=west] at (8.3, 1.5) {
  \begin{tabular}{l}
  \underline{id}: INT (PK) \\
  title: VARCHAR(512) \\
  abstract: TEXT \\
...
% Relationships
\node[rel] (r1) at (2, -1.5) {1:N};
\node[rel] (r2) at (6, -1.5) {1:N};
\node[rel] (r3) at (0, -1.5) {1:N};
...
\code{/chat/stream}          & POST & 是 & 流式SSE多轮对话 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsubsection{对话上下文注入}

\api{POST /chat/stream}接口在转发对话时，自动从\code{user\_clicks}中获取用户最近5篇已读论文的标题和摘要，拼接到系统提示词中，使LLM能根据用户阅读兴趣提供个性化回答。
...
通过DashScope OpenAI兼容接口调用\code{qwen-flash}模型，支持流式和非流式两种模式。业务层在转发前注入包含用户近期阅读论文的系统提示词，使对话具有上下文感知能力。流式响应通过FastAPI \code{StreamingResponse}实现SSE推送。
...
论文详情  & \code{PaperDetail.vue}   & 标题、作者、摘要、关键词、来源链接 \\
论文推荐  & \code{PaperRecommend.vue}& 基于浏览历史的个性化推荐列表 \\
AI对话    & \code{Chat.vue}          & SSE流式Markdown渲染 + 多轮对话 \\
...
\midrule
A1 & 完整系统（向量+Rerank）& 0.900 & 0.858 & 0.696 & 0.919 & 0.979 & 411.4 \\
A2 & SQL LIKE关键词搜索     & 0.600 & 0.563 & 0.444 & 0.621 & 0.805 & 7.9 \\
A3 & TF-IDF余弦相似度       & 0.883 & 0.758 & 0.606 & 0.908 & 1.000 & 0.5 \\
A4 & 向量检索（无Rerank）    & 0.900 & 0.858 & 0.696 & 0.919 & 0.979 & 213.9 \\
A5 & 向量检索（余弦距离）    & 0.900 & 0.858 & 0.696 & 0.919 & 0.979 & 198.3 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{分析}：向量语义检索（A1/A4/A5）在P@K和NDCG@K上显著优于SQL LIKE（A2），提升幅度约30\%--50\%。TF-IDF（A3）效果接近但在P@10上弱于向量检索（0.758 vs 0.858），说明预训练Embedding在语义理解上有明显优势。
...
B3 & 缩短窗口（5篇）    & 0.750 & 0.608 & 0.489 & 10.3 \\
B4 & 不过滤已读         & 0.583 & 0.683 & 0.482 & 10.7 \\
B5 & 质心加随机扰动     & 0.833 & 0.742 & 0.534 & 11.5 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{分析}：过滤已读论文（B1 vs B4）对Hit Rate贡献显著（+16.7\%）。随机扰动（B5）意外提升了命中率和多样性，说明在小数据集上适度随机有利于探索。
...
\midrule
均值 & 0.900 & 0.858 & 0.696 & 0.979 & 0.482 \\
\bottomrule
\end{tabular}
...
\midrule
Embedding编码     & 199.6 & 34.1 \\
ChromaDB检索      & 7.3   & 2.5 \\
Reranker精排      & 8.6   & 0.4 \\
\textbf{端到端总计} & \textbf{215.5} & \textbf{37.0} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{分析}：延迟瓶颈主要在Embedding编码（占92.6\%），ChromaDB向量检索本身仅需约7ms。
...
\midrule
均值 & 0.604 & 0.608 & 0.489 & 6.644 & 11.4 \\
\bottomrule
\end{tabular}
\end{table}
...
Inner Product  & 0.900 & 0.858 & 0.919 & 0.884 & 0.979 & 6.00 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
C1 & 仅Title                    & 0.842 & 0.796 & 0.868 & 0.979 & 0.442 \\
C2 & 仅Abstract                 & 0.892 & 0.846 & 0.896 & 0.958 & 0.448 \\
C3 & 仅Keywords                 & \textbf{0.925} & \textbf{0.867} & \textbf{0.928} & 0.958 & 0.466 \\
C4 & Title + Abstract           & 0.892 & 0.846 & 0.904 & 0.958 & 0.461 \\
C5 & Title + Abstract + Keywords& 0.900 & 0.858 & 0.919 & \textbf{0.979} & \textbf{0.482} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{分析}：完整拼接（C5）在MRR和MAP上最优，综合表现最均衡，故作为系统默认配置。
...
\midrule
R1 & Base Vector（无精排）      & 0.900 & 0.858 & 0.919 & 0.482 & 244.6 \\
R2 & Qwen3-Rerank（召回60）     & \textbf{0.925} & 0.854 & \textbf{0.939} & 0.471 & 569.0 \\
R3 & Qwen3-Rerank（召回100）    & \textbf{0.925} & 0.854 & \textbf{0.939} & 0.473 & 615.4 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{分析}：Qwen3-Rerank在P@5上提升2.5\%，但延迟翻倍（\textasciitilde 570ms vs 245ms）。扩大召回窗口（R3 vs R2）效果几乎无差异，说明60的召回量已足够。
...
\midrule
20  & 1,405  & 1,271  & 4.56 & 6.30 & 7.49 \\
40  & 3,049  & 2,867  & 4.36 & 6.04 & 7.63 \\
60  & 3,865  & 3,672  & 4.37 & 6.41 & 7.85 \\
80  & 5,444  & 5,168  & 3.78 & 7.55 & 10.69 \\
100 & 6,454  & 6,215  & 3.15 & 4.99 & 6.47 \\
120 & 7,802  & 7,576  & 3.18 & 4.67 & 5.85 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{分析}：索引构建时间与论文数量呈线性关系，瓶颈在Embedding编码（API调用），ChromaDB upsert仅占约3\%。查询延迟在120篇规模下稳定在3--6ms，表明系统具有良好的可扩展性。

\subsection{实验小结}

\begin{enumerate}
\item 向量语义检索相比SQL LIKE，P@5提升50\%（0.60 $\to$ 0.90），语义理解能力显著。
\item 推荐系统的已读过滤机制对Hit Rate贡献16.7\%，是推荐效果的关键组件。
\item 系统端到端搜索延迟约215ms，瓶颈在Embedding编码（92.6\%），ChromaDB检索本身仅约7ms。
\item Qwen3-Rerank可提升Top-5精度2.5\%，但延迟增加约一倍，适合对精度要求高的场景。
\item 归一化向量下L2/Cosine/IP三种度量效果完全等价。
\item 完整的Title+Abstract+Keywords拼接是最均衡的文档表示方式。
\end{enumerate}
...
每篇论文包含标题、完整摘要、作者列表、发表会议/期刊、年份、关键词和来源链接，数据来源为OpenAlex学术API。
...
该脚本会将120篇论文写入MySQL并在ChromaDB中建立向量索引。
...
\item \textbf{查看论文}：点击搜索结果中的论文标题，查看论文详情（标题、作者、摘要、关键词）。
\item \textbf{论文推荐}：浏览若干论文后，点击侧边栏"论文推荐"，系统将根据浏览历史推荐相关论文。
\item \textbf{AI对话}：点击侧边栏"AI对话"，与LLM进行多轮学术问答。系统会自动注入近期阅读的论文上下文。
...
\item \textbf{业务层实现}：基于FastAPI实现了完整的RESTful API，包括JWT认证、论文CRUD、搜索/推荐代理和LLM对话转发，并实现了算法层不可用时的SQL降级机制。

\item \textbf{算法层实现}：实现了基于ChromaDB的向量语义检索、Embedding质心推荐和多种Reranker精排策略，支持DashScope Qwen兼容接口的流式LLM对话。

\item \textbf{前端界面}：基于Vue~3 + Element Plus构建了包含搜索、推荐、详情、对话等7个页面的完整前端，支持流式Markdown渲染、搜索历史、路由守卫等特性。

\item \textbf{算法实验}：设计并执行了8组完整实验，涵盖消融分析、性能评估、查询优化和可扩展性测试，验证了向量检索相比关键词搜索P@5提升50\%、推荐系统Hit Rate达0.60、端到端延迟约215ms等核心指标。
\end{enumerate}
